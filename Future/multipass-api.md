...
- Data flow: Scheduler triggers geometry update; renderer.render() gets frame `{ network, timestamp, camera }`. Layers read sparse/indexed typed views and upload/update GPU resources (WebGL: textures/buffers; WebGPU: storage buffers). Instancing/draw count is derived from active index lists. Camera uniforms depend on 2d/3d mode, projection, viewport.
