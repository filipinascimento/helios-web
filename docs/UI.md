...
The Scene panel’s Appearance tab now includes a `Shaded` subsection with a header toggle that enables or disables shader-specialized lighting globally. Inside that subsection, `Nodes` shading defaults to on, `Edges` shading defaults to off, diffuse strength defaults to `0.5`, ambient-top defaults to white, ambient-bottom defaults to `rgb(163, 163, 163)`, ambient strength defaults to `1.0`, and specular strength defaults to `0.0`. You can tune the light direction, shaded colors, strengths, and shininess. When the header toggle is off, the extra shaded shader paths and bindings are omitted from the compiled render variants.
...
The Mappers panel’s Density tab supports both the legacy `Difference` mode and the new `Log Ratio` mode. `Difference` preserves the existing normalized comparison path, while `Log Ratio` switches to a specialized dual-density comparison path with a real-valued numeric legend. In `Log Ratio` mode, the density panel exposes `Epsilon`, `Range`, `Z-score`, and `Support` controls and disables the legacy `Weight` / `Norm.` controls because those would break the interpretation of the numeric colorbar. `Z-score` switches the display from the raw log-ratio to a fast approximate local z-score derived from the same two density fields. `Support` enables or disables the automatic pooled-support correction that fades unstable sparse tails without changing the raw values in well-supported areas. Density comparison colormap picking keeps the active search/filter state between openings, highlights the selected colormap, and includes a `Diverging` filter inside the picker.
