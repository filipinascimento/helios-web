...
The Scene panel’s Appearance tab now includes a `Shaded` subsection with a header toggle that enables or disables shader-specialized lighting globally. Inside that subsection, `Nodes` shading defaults to on, `Edges` shading defaults to off, and you can tune the light direction plus diffuse, ambient-top, ambient-bottom, and specular colors alongside diffuse strength, ambient strength, specular strength, and shininess. When the header toggle is off, the extra shaded shader paths and bindings are omitted from the compiled render variants.
