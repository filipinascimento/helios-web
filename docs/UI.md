...
Categorical node-color legend rows can be hovered to highlight matching nodes and connected edges, clicked to replace the selection, and Shift-clicked to add or remove categories. Clicking an already selected category toggles it off. Hovered rows show a gray outline; fully selected categories keep a theme-aware gray outline without changing label size or style. Density defaults to `interactionFilter: 'auto'`, which focuses on selected nodes first, then real highlighted nodes, then all active nodes.
