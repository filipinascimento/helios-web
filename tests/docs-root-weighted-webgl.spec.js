...
test('root demo renders something with WebGL indirect + weighted in 3D', async ({ page }) => {
  await page.goto('/?nodes=20000&mode=3d&edgeTransparency=weighted&renderer=webgl&webglBackend=indirect&layout=none');
  await page.waitForFunction(() => window.__HELIOS_DIAGNOSTICS__?.ready === true, null, { timeout: 30_000 });
  await page.waitForTimeout(750);
