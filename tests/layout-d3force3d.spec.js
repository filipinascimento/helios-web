...
test.describe('d3-force-3d layout behavior', () => {
  test('connected nodes end up closer than random non-neighbors', async ({ page }, testInfo) => {
    await page.goto('/tests/fixtures/demo.html?layout=d3force3d&mode=2d&nodes=240');
    ...
    const stats = await page.evaluate(() => {
      ...
      return helios.network.withBufferAccess(() => {
        const pos = helios.network.getNodeAttributeBuffer('_helios_visuals_position').view;
    ...
    await testInfo.attach('d3force3d-layout-stats', {
    ...
    expect(stats.nonEdgeAvg).toBeGreaterThanOrEqual(stats.edgeAvg * 0.9);
  });
});
