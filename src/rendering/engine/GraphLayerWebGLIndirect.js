...
import { createGraphWebGLIndirectSources } from './shaders/graphWebGLIndirect.js';
...
export class GraphLayerWebGLIndirect extends GraphLayer {
  constructor(options = {}) {
    super(options);
    this.device = null;
    this.gl = null;
    this.isIndirectWebGL = true;
  ...
  initialize(device, size) {
    if (device?.type !== 'webgl2') {
      throw new Error('GraphLayerWebGLIndirect requires a WebGL2 device.');
    }
  ...
  resolveIndirectNodeVariant(visualConfig) {
  ...
  getNodeProgram(nodeVariant) {
    ...
    return this.getProgramEntry(
      ...
      key,
      () => {
        const sources = createGraphWebGLIndirectSources(this.buildIndirectShaderOptions(nodeVariant, null));
  ...
  getEdgeProgram(kind, nodeVariant, edgeVariant, weighted = false) {
    ...
    return this.getProgramEntry(
      ...
      key,
      () => {
        const sources = createGraphWebGLIndirectSources(this.buildIndirectShaderOptions(nodeVariant, edgeVariant));
        return {
  ...
  ensureWeightedTargets(width, height) {
    ...
    if (status !== gl.FRAMEBUFFER_COMPLETE) {
      console.warn('GraphLayerWebGLIndirect: weighted framebuffer incomplete, falling back to alpha.', status);
      this.destroyWeightedTargets();
  ...
  withSparseGraph(network, topologyVersions, indices, edgeNodeAttributes, fn) {
    ...
    if (typeof network.withBufferAccess !== 'function') {
      console.warn('GraphLayerWebGLIndirect: network does not support buffer access sessions');
      return false;
    }
  ...
  render(context, frame) {
    ...
    const nodeVariant = this.resolveIndirectNodeVariant(visualConfig);
      ...
      (geometry) => {
        ...
        if (weightedReady) {
          if (!this.loggedWeightedActive) {
            console.info(`GraphLayerWebGLIndirect: using weighted multipass for '${transparencyMode}'`);
...
export default GraphLayerWebGLIndirect;
